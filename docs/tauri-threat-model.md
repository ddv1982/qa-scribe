...
- Provider CLIs run in fresh private working directories that are removed after execution. Their stdout is bounded at both the process-reader and core aggregation layers, individual event lines have a lower bound, and stderr is fully drained while only a bounded tail is retained for diagnostics.
- Provider readiness probes use private temporary directories and files so prompts and captured output do not inherit broad default filesystem permissions.
