...
The scheduled boot work now runs `getProviderStatus` only. The backend Fast status path detects executable presence (`src-tauri/src/commands/providers.rs`, `src-tauri/src/commands/providers/detection.rs`). Explicit Deep refresh clears readiness and PATH caches, then performs deeper provider checks without blocking Tauri's async command executor (`src-tauri/src/commands/providers.rs`). Deep detection runs the three independent provider checks concurrently and restores stable provider order before returning; each provider can run version, model, auth, and help probes (`src-tauri/src/commands/providers/detection.rs`, `src-tauri/src/commands/providers/models.rs`). Each provider subprocess probe has a 4 second timeout (`src-tauri/src/commands/providers/probe.rs`), and login-shell PATH probing has a 2 second timeout (`src-tauri/src/provider_command.rs`).
